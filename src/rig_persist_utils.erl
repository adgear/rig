...
-export([to_persistent_term/1, find_records/1, divide/2]).
...
-spec divide(list(), pos_integer()) -> list().
...
build_persist_map_test() ->
    L = build_persist_map(deep_lists()),
    %io:format("L:~p~n",[L]),
    ?assertEqual(6, length(L)).
